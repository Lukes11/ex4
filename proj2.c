...
static char linkedList[50], hashTable[50], redBlackTree[50], radixTree[50], xArray[50];
...
{ 
	...
	struct xEntry *x1 = kmalloc(sizeof(*x1), GFP_KERNEL);
	...
	else 
	{
		...
		//add too XArray
		x1->val = val;
		xa_store(&myXArray, xArrIndex, &x1->val, GFP_KERNEL);
		xArrIndex++;
		return 0;
	}
}
...
static void test_data_structures(void)
{
	...
	strcpy(xArray, structureValues);

}

static void destroy_structures_and_free(void)
{
...
static void cleanup(void)
{
	...
	destroy_structures_and_free();
}

//Create proc entry
static int structures_proc_show(struct seq_file *m, void *v) {
	seq_printf(m, "%s\n%s\n%s\n%s\n%s\n", linkedList, hashTable, redBlackTree, radixTree, xArray);
	printk(KERN_INFO "%s\n%s\n%s\n%s\n%s\n", linkedList, hashTable, redBlackTree, radixTree, xArray);
	return 0;
}
...
static int __init ex4_init(void)
{
	...
	run_tests();
	proc_create("proj2", 0, NULL, &structures_proc_fops);
out:
